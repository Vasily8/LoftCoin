package com.vasily.loftcoin.job;
...
public class JobHelperImpl implements JobHelper {
    ...
    public void startSyncRateJob(String symbol) {
        ...
        JobInfo jobInfo = new JobInfo.Builder(SYNC_RATE_JOB_ID, componentName)
                .setRequiredNetworkType(JobInfo.NETWORK_TYPE_ANY) // network type for any connection
                .setRequiresDeviceIdle(false) // non-phone mode
                .setRequiresCharging(false) //on charge
                .setExtras(extras)
                .setPersisted(true) // start after reboot
                .build();
